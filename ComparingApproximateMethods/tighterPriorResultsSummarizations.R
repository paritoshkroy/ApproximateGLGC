...
scores_df_list <- lapply(1:length(fname), function(node){
  load(fname[node])
  cat("node ", node, "\n")
  if(exists("scores_df") & exists("sampler_diag")){
  scores_df <- scores_df %>% 
    mutate(`Elapsed Time (in hours)` = scores_df$`Elapsed Time`/3600) %>%
    mutate(node =  as.numeric(gsub(".*?([0-9]+).*", "\\1", fname[node]))) %>%
...
save(fixed_summary_df, scores_df, file = "tighterPriorResultsSummarizations.RData")
...
load("./ComparingApproximateMethods/tighterPriorResultsSummarizations.RData")
...
ggsave(filename = "./ComparingApproximateMethods/tighterPrior_replicated_fixed_parameters.png", height = 9, width = 11)
...
scores_df_long <- scores_all_df %>% select(-node, -ID, -n) %>%
  gather(Key, Value, -Model)
  
table(scores_df_long$Key)
...
ggsave(filename = "./ComparingApproximateMethods/tighterPrior_replicated_scoring_rules.png", height = 6, width = 11)
