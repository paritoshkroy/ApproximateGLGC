...
model {
  ...
  tau ~ std_normal();
  noise ~ std_normal();
  y ~ vecchia_matern32(X * beta + gamma * exp(latent_nngp_matern32_stuff(noise, square(sigma1), lscale1, site2neiDist, neiDistMat, neiID, N, K)), square(sigma2), square(tau), lscale2, site2neiDist, neiDistMat, neiID, N, K);
}
