...
input <- list(N = nsize, P = P, y = obsY, X = obsX, coords = obsCoords, mu_beta = mu_beta, V_beta = V_beta, a = ab[1], b = ab[2])
str(input)

library(cmdstanr)
stan_file <- paste0(fpath,"StanFiles/fullGLGC.stan")
...
save(elapsed_time, fixed_summary, draws_df, z_summary, file = paste0(fpath,"ExactVsApproximateMethods/fullGLGC.RData"))
...
pred_summary <- cbind(
  ...
  q2.5 = apply(ypred_draws, 2, quantile2.5),
  q50 = apply(ypred_draws, 2, quantile50),
  q97.5 = apply(ypred_draws, 2, quantile97.5),
  y = prdY)
head(pred_summary)
...
save(elapsed_time, fixed_summary, draws_df, z_summary, pred_summary, scores_df, file = paste0(fpath,"ExactVsApproximateMethods/fullGLGC.RData"))
