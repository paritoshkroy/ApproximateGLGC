...
save(elapsed_time, fixed_summary, draws_df, z1_summary, file = paste0(fpath,"ExactVsApproximateMethods/Full_DataSet4.RData"))
...
scores_df <- pred_summary %>% 
  ...
  summarise(MAE = sqrt(mean(abs(error))), RMSE = sqrt(mean(error^2)), CVG = mean(btw),
            IS = mean(intervals)) %>%
  mutate(ES = ES, logs = logs, CRPS = CRPS,  `Elapsed Time` = elapsed_time$total, Method = "Full_DataSet4") %>%
  select(Method,MAE,RMSE,CVG,CRPS,IS,ES,logs,`Elapsed Time`)
scores_df

save(elapsed_time, fixed_summary, draws_df, z1_summary, post_z1, yfitted_summary, pred_summary, scores_df, file = paste0(fpath,"ExactVsApproximateMethods/Full_DataSet4.RData"))
...
ggplot(z_summary) +
  ...
  theme(panel.grid = element_blank(),
        legend.position = c(0.2,0.8),
        legend.title = element_blank())
ggsave(paste0(fpath,"ExactVsApproximateMethods/Full_DataSet4_SpatialEffect_Density.png"), height = 4, width = 6)

save(elapsed_time,scores_df, z_summary, yfitted_summary, kde_df, fixed_summary, draws_df, file = paste0(fpath,"ExactVsApproximateMethods/Full_DataSet4.RData"))
