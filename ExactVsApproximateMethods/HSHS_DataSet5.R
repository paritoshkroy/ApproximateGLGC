...
z_summary
save(elapsed_time, fixed_summary, draws_df, z1_summary, z2_summary, z_summary, file = paste0(fpath,"ExactVsApproximateMethods/HSHS_DataSet5.RData"))
...
scores_df <- pred_summary %>% 
  ...
  summarise(MAE = sqrt(mean(abs(error))), RMSE = sqrt(mean(error^2)), CVG = mean(btw),
            IS = mean(intervals)) %>%
  mutate(ES = ES, logs = logs, CRPS = CRPS,  `Elapsed Time` = elapsed_time$total, Method = "HSHS_DataSet5") %>%
  select(Method,MAE,RMSE,CVG,CRPS,IS,ES,logs,`Elapsed Time`)
scores_df

save(sampler_diag, elapsed_time, fixed_summary, draws_df, z1_summary, z2_summary, z_summary, yfitted_summary, post_z, pred_summary, scores_df, file = paste0(fpath,"ExactVsApproximateMethods/HSHS_DataSet5.RData"))
...
ggsave(paste0(fpath,"ExactVsApproximateMethods/HSHS_DataSet5_SpatialEffect_Density.png"), height = 4, width = 6)

save(sampler_diag, kde_df, elapsed_time, fixed_summary, draws_df, z1_summary, z2_summary, z_summary, post_z, yfitted_summary, pred_summary, scores_df, file = paste0(fpath,"ExactVsApproximateMethods/HSHS_DataSet5.RData"))
