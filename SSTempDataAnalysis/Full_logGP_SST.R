...
input <- list(N = nsize, P = P, y = log(obsY), X = obsX, coords = obsCoords, mu_theta = mu_theta, V_theta = V_theta, lambda_sigma2 = lambda_sigma2, lambda_tau = lambda_tau, a = ab[1], b = ab[2])
str(input)

library(cmdstanr)
stan_file <- paste0(fpath,"StanFiles/Full_GP_Exp.stan")
...
## Posterior summaries
pars <- c(paste0("theta[",1:P,"]"),"sigma2","ell2","tau")
...
save(elapsed_time, fixed_summary, draws_df, file = paste0(fpath,"SSTempDataAnalysis/Full_logGP_SST.RData"))
...
post_logypred <- exsf$predict_fullgp_rng(
  y = log(obsY), 
...
ypred_draws <- do.call(rbind,lapply(post_logypred, exp)); str(ypred_draws)
pred_summary <- tibble(
...
scores_df <- pred_summary %>% filter(!is.na(y)) %>%
  ...
  summarise(MAE = sqrt(mean(abs(error))), RMSE = sqrt(mean(error^2)), CVG = mean(btw),
            IS = mean(intervals)) %>%
  mutate(ES = ES, logs = logs, CRPS = CRPS,  `Elapsed Time` = elapsed_time$total, Method = "Full_logGP") %>%
  select(Method,MAE,RMSE,CVG,CRPS,IS,ES,logs,`Elapsed Time`)
scores_df

save(elapsed_time, fixed_summary, draws_df, pred_summary, scores_df, file = paste0(fpath,"SSTempDataAnalysis/Full_logGP_SST.RData"))
