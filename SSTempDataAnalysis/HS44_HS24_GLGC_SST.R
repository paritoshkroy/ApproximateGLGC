...
################################################################################
quantile(obsDistVec, probs = seq(0,1,l=21))
...
m1 <- ceiling(3.42 * 1.25/(minimum_identifiable_lscale/Lstar[1])); m1
m2 <- ceiling(3.42 * 1.25/(minimum_identifiable_lscale/Lstar[2])); m2
mstar <- m1*m2; mstar
c <- c(1.25,1.25)
L <- c*Lstar; L
str(L)
...
z_summary
save(elapsed_time, fixed_summary, draws_df, z1_summary, z2_summary, z_summary, file = paste0(fpath,"SSTempDataAnalysis/HS44_HS24_GLGC_SST.RData"))
...
scores_df <- pred_summary %>% filter(!is.na(y)) %>%
  ...
  summarise(MAE = sqrt(mean(abs(error))), RMSE = sqrt(mean(error^2)), CVG = mean(btw),
            IS = mean(intervals)) %>%
  mutate(ES = ES, logs = logs, CRPS = CRPS,  `Elapsed Time` = elapsed_time$total, Method = "HS44_HS24_GLGC") %>%
  select(Method,MAE,RMSE,CVG,CRPS,IS,ES,logs,`Elapsed Time`)
scores_df

save(elapsed_time, fixed_summary, draws_df, z1_summary, z2_summary, z_summary, pred_summary, scores_df, file = paste0(fpath,"SSTempDataAnalysis/HS44_HS24_GLGC_SST.RData"))
