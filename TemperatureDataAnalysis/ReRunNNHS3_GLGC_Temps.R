...
save(file = paste0(fpath,"TemperatureDataAnalysis/ImageReRunNNHS3_GLGC_Temps.RData"))
...
id_full_missing <- which(is.na(prdY[selectedID])); str(id_full_missing)
library(scoringRules)
ES <- es_sample(y = prdY[selectedID][-id_full_missing], dat = t(ypred_draws)[-id_full_missing,]); ES
logs <- mean(logs_sample(y = prdY[selectedID][-id_full_missing], dat = t(ypred_draws)[-id_full_missing,])); logs
CRPS <- mean(crps_sample(y = prdY[selectedID][-id_full_missing], dat = t(ypred_draws)[-id_full_missing,])); CRPS

scores_df <- pred_summary %>% filter(!is.na(y)) %>%
  mutate(intervals = scoringutils::interval_score(true_values = y, lower = post.q2.5, upper = post.q50, interval_range = 95)) %>%
  ...
  summarise(MAE = sqrt(mean(abs(error))), RMSE = sqrt(mean(error^2)), CVG = mean(btw),
            IS = mean(intervals)) %>%
  mutate(ES = ES, logs = logs, CRPS = CRPS,  `Elapsed Time` = elapsed_time$total, Method = "NNHS3_GLGC") %>%
  select(Method,MAE,RMSE,CVG,CRPS,IS,ES,logs,`Elapsed Time`)
scores_df

save(nNeighbors, obsCoords, prdCoords, post_z1, yfitted_summary, fit_summary, m1, m2, mstar, elapsed_time, fixed_summary, draws_df, z1pred_summary, pred_summary, scores_df, prdCoords, prdCoords.full, selectedID, file = paste0(fpath,"TemperatureDataAnalysis/ReRunNNHS3_GLGC_Temps.RData"))
...
ggplot(z_summary) +
  ...
  theme(panel.grid = element_blank(),
        legend.position = c(0.2,0.8),
        legend.title = element_blank())
#ggsave(paste0(fpath,"TemperatureDataAnalysis/NNHS3_GLGC_Temps_SpatialEffect_Density.png"), height = 4, width = 6)

save(nNeighbors, obsCoords, prdCoords, yfitted_summary, fit_summary, sampler_diag, m1, m2, mstar, elapsed_time, fixed_summary, draws_df, z_summary, kde_df, z1_summary, z2_summary, z1pred_summary, pred_summary, scores_df, prdCoords, prdCoords.full, selectedID, file = paste0(fpath,"TemperatureDataAnalysis/ReRunNNHS3_GLGC_Temps.RData"))
