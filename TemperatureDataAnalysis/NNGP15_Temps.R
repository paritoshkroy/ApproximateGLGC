...
input <- list(N = nsize, K = nNeighbors, P = 2, y = obsY, X = obsX, coords = obsCoords, neiID = neiMatInfo$NN_ind, site2neiDist = neiMatInfo$NN_dist, neiDistMat = neiMatInfo$NN_distM, mu_theta = mu_theta, V_theta = V_theta, a = ab[1], b = ab[2])
str(input)
...
save(elapsed_time, fixed_summary, draws_df, file = paste0(fpath,"TemperatureDataAnalysis/NNGP15_Temps.RData"))
...
post_ypred <- exsf$predict_vecchia_rng(
  y = obsY, 
  obsX = obsX, 
  predX = prdX, 
...
scores_df <- pred_summary %>% filter(!is.na(y)) %>%
  ...
  summarise(MAE = sqrt(mean(abs(error))), RMSE = sqrt(mean(error^2)), CVG = mean(btw),
            IS = mean(intervals)) %>%
  mutate(ES = ES, logs = logs, CRPS = CRPS,  `Elapsed Time` = elapsed_time$total, Method = "NNGP15") %>%
  select(Method,MAE,RMSE,CVG,CRPS,IS,ES,logs,`Elapsed Time`)
scores_df

save(elapsed_time, fixed_summary, draws_df, pred_summary, scores_df, file = paste0(fpath,"TemperatureDataAnalysis/NNGP15_Temps.RData"))
