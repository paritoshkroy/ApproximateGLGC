...
input <- list(N = nsize, M = mstar, P = P, y = log(obsY), X = obsX, coords = obsCoords, L = L, lambda = lambda, mu_theta = mu_theta, V_theta = V_theta, a = ab[1], b = ab[2], lambda_sigma = lambda_sigma, lambda_tau = lambda_tau)
str(input)
...
save(elapsed_time, fixed_summary, draws_df, z_summary, file = paste0(fpath,"TemperatureDataAnalysis/logHSGP2_Temps.RData"))
...
str(post_zpred)
ypred_draws <- t(sapply(1:size_post_samples, function(l) exp(prdXtheta[l,] + post_zpred[l,] + rnorm(n = psize, mean = 0, sd = post_tau[l]))))
str(ypred_draws)
...
scores_df <- pred_summary %>% filter(!is.na(y)) %>%
  ...
  summarise(MAE = sqrt(mean(abs(error))), RMSE = sqrt(mean(error^2)), CVG = mean(btw),
            IS = mean(intervals)) %>%
  mutate(ES = ES, logs = logs, CRPS = CRPS,  `Elapsed Time` = elapsed_time$total, Method = "logHSGP2") %>%
  select(Method,MAE,RMSE,CVG,CRPS,IS,ES,logs,`Elapsed Time`)
scores_df

save(elapsed_time, fixed_summary, draws_df, z_summary, pred_summary, scores_df, file = paste0(fpath,"TemperatureDataAnalysis/logHSGP2_Temps.RData"))
