...
apply(selected.sat.temps[,c("relocateLon","relocateLat")], 2, range)
#####################################################################
...
obsCoords <- selected.sat.temps %>% filter(!is.na(MaskTemp)) %>% select(relocateLon,relocateLat) %>% as.matrix() %>% unname(); str(obsCoords)
prdCoords <- selected.sat.temps %>% filter(is.na(MaskTemp)) %>% select(relocateLon,relocateLat) %>% as.matrix() %>% unname(); str(prdCoords)
...
################################################################################
xyRanges <- apply(selected.sat.temps[,c("relocateLon","relocateLat")], 2, range); xyRanges
