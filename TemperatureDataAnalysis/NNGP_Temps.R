...
args(exsf$vecchia_matern32_fitted_rng)
l <- 1
yfitted_list <- lapply(1:size_post_samples, function(l) exsf$vecchia_matern32_fitted_rng(y = obsY, mu = obsXtheta[l,], sigmasq = post_sigma[l]^2, tausq = post_tau[l]^2, lscale = post_ell[l], site2neiDist = neiMatInfo$NN_dist, neiDistMat = neiMatInfo$NN_distM, neiID = lapply(1:nrow(neiMatInfo$NN_ind), function(i) neiMatInfo$NN_ind[i,]), N = nsize, K = nNeighbors))
...
save(elapsed_time, obsCoords, prdCoords, yfitted_summary, fit_summary, fixed_summary, draws_df, pred_summary, scores_df, file = paste0(fpath,"TemperatureDataAnalysis/NNGP_Temps.RData"))
