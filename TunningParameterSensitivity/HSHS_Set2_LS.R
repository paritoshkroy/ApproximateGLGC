...
node <- as.numeric(args[1])-120 ### specify correct node here
...
vector_m <- pmax(round(3.42*vector_c/vector_lscale,0),32); vector_m
...
z_summary
save(elapsed_time, fixed_summary, draws_df, z1_summary, z2_summary, z_summary, file = paste0(fpath,"TunningParameterSensitivity/HSHS_Set2_LS",node,".RData"))
...
scores_df <- pred_summary %>% 
  ...
  summarise(MAE = sqrt(mean(abs(error))), RMSE = sqrt(mean(error^2)), CVG = mean(btw),
            IS = mean(intervals)) %>%
  mutate(ES = ES, logs = logs, CRPS = CRPS,  `Elapsed Time` = elapsed_time$total, Method = paste0("HSHS_Set2_LS",node)) %>%
  select(Method,MAE,RMSE,CVG,CRPS,IS,ES,logs,`Elapsed Time`)
scores_df

save(elapsed_time, fixed_summary, draws_df, z1_summary, z2_summary, z_summary, pred_summary, scores_df, file = paste0(fpath,"TunningParameterSensitivity/HSHS_Set2_LS",node,".RData"))
