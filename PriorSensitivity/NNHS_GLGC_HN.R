...
save(elapsed_time, fixed_summary, draws_df, z1_summary, file = paste0(fpath,"PriorSensitivity/NNHS_GLGC_HN.RData"))
...
scores_df <- pred_summary %>% 
  ...
  summarise(MAE = sqrt(mean(abs(error))), RMSE = sqrt(mean(error^2)), CVG = mean(btw),
            IS = mean(intervals)) %>%
  mutate(ES = ES, logs = logs, CRPS = CRPS,  `Elapsed Time` = elapsed_time$total, Method = "NNHS_GLGC_HN") %>%
  select(Method,MAE,RMSE,CVG,CRPS,IS,ES,logs,`Elapsed Time`)
scores_df

save(elapsed_time, fixed_summary, draws_df, z1_summary, pred_summary, scores_df, post_z1, file = paste0(fpath,"PriorSensitivity/NNHS_GLGC_HN.RData"))
